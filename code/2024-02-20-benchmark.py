...
template<int... Is> struct Sort<Vector<Is...>> {
  ...
  using type = Helper<Iota<sizeof...(Is)>>::type;
        ...
        step_code = {
            5: "std::sort(arr.data, arr.data + sizeof...(Is));",
            6: "std::ranges::sort(arr.data);",
        }[x]
        return """
template<int N> struct Array { int data[N]; };
template<class> struct Sort;
template<int... Is> struct Sort<Vector<Is...>> {
  static constexpr Array<sizeof...(Is)> sorted = []() {
...
def scatter_one_plot(plt, nouns, verbs, data):
    ...
    marker, color, label = {
        ...
        (5, 0): ('^', '#d62728', 'Vector, Array, std::sort'),
        (6, 0): ('<', '#ff7f0e', 'Vector, Array, std::ranges::sort'),
        ...
        (5, 1): ('v', '#d62728', 'std::integer_sequence, Array, std::sort'),
        (6, 1): ('>', '#ff7f0e', 'std::integer_sequence, Array, std::ranges::sort'),
    }[(verbs, nouns)]
